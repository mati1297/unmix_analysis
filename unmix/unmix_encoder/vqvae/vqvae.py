...
from unmix_encoder.vqvae.encdec import Encoder, Decoder, assert_shape
from unmix_encoder.vqvae.bottleneck import NoBottleneck, Bottleneck
from unmix_encoder.utils.logger import average_metrics
from unmix_encoder.utils.audio_utils import spectral_convergence, spectral_loss, multispectral_loss, audio_postprocess
...
class VQVAE_Encoder(nn.Module):
    ...
    def encode(self, x, start_level=0, end_level=None, bs_chunks=1):
        ...
        x_encod_list = []
        for x_i in x_chunks:
            x_encod = self._encode(
                x_i, start_level=start_level, end_level=end_level)
            x_encod_list.append(x_encod)
        xs = [t.cat(xs_level_list, dim=0)
              for xs_level_list in zip(*x_encod_list)]
        return xs
...
class VQVAE_Unmix_Only_Encoder(nn.Module):
    def __init__(self, input_shape, levels, downs_t, strides_t,
                 emb_width, l_bins, mu, commit, spectral, multispectral,
                 multipliers=None, use_bottleneck=True, **block_kwargs):
        super().__init__()
