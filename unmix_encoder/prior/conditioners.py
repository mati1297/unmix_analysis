...
from unmix_encoder.vqvae.encdec import DecoderConvBock
...
class Conditioner(nn.Module):
    def __init__(self, input_shape, bins, down_t, stride_t, out_width, init_scale, zero_out, res_scale, **block_kwargs):
        ...
        # Conditioner
        self.cond = DecoderConvBock(self.width, self.width, down_t, stride_t,
                                    **block_kwargs, zero_out=zero_out, res_scale=res_scale)
...
class SimpleEmbedding(nn.Module):
    ...
    def forward(self, y):
        ...
        assert isinstance(
            y, t.cuda.LongTensor), f"Expected dtype {t.cuda.LongTensor}, got {y.dtype}"
...
class RangeEmbedding(nn.Module):
    ...
    def forward(self, pos_start, pos_end=None):
        ...
        assert len(
            pos_start.shape) == 2, f"Expected shape with 2 dims, got {pos_start.shape}"
        ...
        if pos_end is not None:
            assert len(
                pos_end.shape) == 2, f"Expected shape with 2 dims, got {pos_end.shape}"
            if self.clamp:
        ...
        if n_time != 1:
            ...
            interpolation = (t.arange(0, n_time, dtype=t.float,
                                      device='cuda').view(1, n_time)/n_time)
            position = pos_start + (pos_end - pos_start)*interpolation
        else:
            position = pos_start

        # Bin each value to bins
        normalised_position = (position - self.pos_min) / \
            (self.pos_max - self.pos_min)  # [0,1)
...
class LabelConditioner(nn.Module):
    def __init__(self, y_bins, t_bins, sr, min_duration, max_duration, n_time, out_width, init_scale, max_bow_genre_size, include_time_signal):
        ...
        assert len(
            y_bins) == 2, f"Expecting (genre, artist) bins, got {y_bins}"
        bow_genre_bins, artist_bins = y_bins
        self.max_bow_genre_size = max_bow_genre_size
        self.bow_genre_emb = SimpleEmbedding(
            bow_genre_bins, out_width, init_scale)
        ...
        if self.include_time_signal:
            ...
            assert len(
                t_ranges) == 3, f"Expecting (total, absolute, relative) ranges, got {t_ranges}"
    ...
    def forward(self, y):
        assert len(y.shape) == 2, f"Expected shape with 2 dims, got {y.shape}"
        assert y.shape[-1] == 4 + \
            self.max_bow_genre_size, f"Expected shape (N,{4 + self.max_bow_genre_size}), got {y.shape}"
        assert isinstance(
            y, t.cuda.LongTensor), f"Expected dtype {t.cuda.LongTensor}, got {y.dtype}"
        N = y.shape[0]
        total_length, offset, length, artist, genre = y[:,
                                                        0:1], y[:, 1:2], y[:, 2:3], y[:, 3:4], y[:, 4:]
        ...
        genre_emb = (self.bow_genre_emb(genre.clamp(0))
        ...
        if self.include_time_signal:
            ...
            pos_emb = self.total_length_emb(total_length) + self.absolute_pos_emb(
                start, end) + self.relative_pos_emb(start/total_length, end/total_length)
