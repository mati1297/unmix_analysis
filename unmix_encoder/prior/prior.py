...
from unmix_encoder.transformer.ops import LayerNorm
from unmix_encoder.prior.autoregressive import ConditionalAutoregressive2D
from unmix_encoder.prior.conditioners import Conditioner, LabelConditioner
from unmix_encoder.data.labels import EmptyLabeller, Labeller

from unmix_encoder.utils.torch_utils import assert_shape
from unmix_encoder.utils.dist_utils import print_once
from unmix_encoder.vqvae.vqvae import calculate_strides
...
class SimplePrior(nn.Module):
    def __init__(self, z_shapes, l_bins, encoder, decoder, level,
        ...
        if self.x_cond:
            ...
            def conditioner_block(_level): return Conditioner(input_shape=z_shapes[_level],
                                                              bins=l_bins,
        ...
        if single_enc_dec:
            ...
            self.prior_shapes = [
                (self.n_tokens,), prior_kwargs.pop('input_shape')]
        ...
        self.cond_downsample = self.downsamples[level +
                                                1] if level != self.levels - 1 else None
    ...
    def get_z_conds(self, zs, start, end):
        if self.level != self.levels - 1:
            ...
            z_cond = zs[self.level + 1][:, start //
                                        self.cond_downsample:end//self.cond_downsample]
    ...
    def x_emb(self, z_conds):
        ...
        assert len(z_conds) == len(self.conditioner_blocks) == self.cond_level - \
            self.level, f"Expected {len(z_conds)} == {len(self.conditioner_blocks)} == {self.cond_level} - {self.level}"
        x_cond = None
    ...
    def encode(self, x, start_level=None, end_level=None, bs_chunks=1):
        ...
        # Get latents
        with t.no_grad():
            zs = self.encoder(x, start_level=start_level,
                              end_level=end_level, bs_chunks=bs_chunks)
        return zs
    ...
    def decode(self, zs, start_level=None, end_level=None, bs_chunks=1):
        ...
        with t.no_grad():
            x_out = self.decoder(zs, start_level=start_level,
                                 end_level=end_level, bs_chunks=bs_chunks)
        return x_out

    def get_cond(self, z_conds, y):
        if y is not None:
            assert y.shape[1] == 4 + self.y_emb.max_bow_genre_size + \
                self.n_tokens, f"Expected {4} + {self.y_emb.max_bow_genre_size} + {self.n_tokens}, got {y.shape[1]}"
    ...
    def get_prime_loss(self, encoder_kv, prime_t):
        if self.use_tokens:
            ...
            prime_loss = nn.functional.cross_entropy(
                encoder_kv.view(-1, self.prime_bins), prime_t.view(-1)) / np.log(2.)
        else:
    ...
    def z_forward(self, z, z_conds=[], y=None, fp16=False, get_preds=False, get_attn_weights=False):
        ...
        if self.single_enc_dec:
            ...
            (prime_loss, gen_loss), preds = self.prior(z, x_cond,
                                                       y_cond, fp16=fp16, get_sep_loss=True, get_preds=get_preds)
        else:
    ...
    def forward(self, x, y=None, fp16=False, decode=False, get_preds=False):
        ...
        loss, metrics = self.z_forward(
            z=z, z_conds=z_conds, y=y, fp16=fp16, get_preds=get_preds)
        if decode:
