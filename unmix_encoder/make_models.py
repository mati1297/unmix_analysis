...
from unmix_encoder.hparams import Hyperparams, setup_hparams, REMOTE_PREFIX
from unmix_encoder.utils.remote_utils import download
from unmix_encoder.utils.torch_utils import freeze_model
...
def save_checkpoint(logger, name, model, opt, metrics, hps):
    with t.no_grad():
        save_hps = {**hps}
        save_hps = {k: v for k, v in save_hps.items() if k not in [
            'metadata_v2', 'metadata_v3', 'alignments', 'lyric_processor', 'midi_processor']}
        t.save({'hps': save_hps,
...
def make_vqvae(hps, device='cuda'):
    from unmix_encoder.vqvae.vqvae import VQVAE
    ...
    if not hps.sample_length:
        ...
        hps.sample_length = (hps.sample_length_in_seconds *
...
def make_encoder(hps, device='cuda'):
    from unmix_encoder.vqvae.vqvae import VQVAE_Encoder
    block_kwargs = dict(width=hps.width, depth=hps.depth, m_conv=hps.m_conv,
                        dilation_growth_rate=hps.dilation_growth_rate,
                        dilation_cycle=hps.dilation_cycle,
                        reverse_decoder_dilation=hps.vqvae_reverse_decoder_dilation)
    ...
    vqvae = VQVAE_Encoder(input_shape=(hps.sample_length, 1), levels=hps.levels, downs_t=hps.downs_t, strides_t=hps.strides_t,
                          emb_width=hps.emb_width, l_bins=hps.l_bins,
                          mu=hps.l_mu, commit=hps.commit,
                          spectral=hps.spectral, multispectral=hps.multispectral,
                          multipliers=hps.hvqvae_multipliers, use_bottleneck=hps.use_bottleneck,
    ...
    vqvae.step = 0
    # no restart at first
    #restore_model(hps, vqvae, hps.restore_encoder)
    restore_model(
...
def make_prior(hps, vqvae, device='cuda'):
    from unmix_encoder.prior.prior import SimplePrior
    ...
    def rescale(z_shape): return (
        z_shape[0]*hps.n_ctx//vqvae.z_shapes[hps.level][0],)
    ...
    if hps.fp16_params:
        ...
        from unmix_encoder.transformer.ops import _convert_conv_weights_to_fp16
...
def save_outputs(model, device, hps):
    ...
    lyric_tokens = t.randint(0, prime_bins, (1, n_tokens),
                             generator=rng, dtype=t.long).view(-1).numpy()
    artist_id = 10
    genre_ids = [1]
    ...
    x_ds = [vq_prior.decode(zs[level:], start_level=level)
    ...
    for level in range(len(priors)):
        ...
        y_in = t.from_numpy(prior.labeller.get_y_from_ids(
            artist_id, genre_ids, lyric_tokens, total_length, offset)).view(1, -1).cuda().long()
