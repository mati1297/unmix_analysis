import torch as t
import numpy as np
from unmix_encoder.data.artist_genre_processor import ArtistGenreProcessor
from unmix_encoder.data.text_processor import TextProcessor
...
def get_relevant_lyric_tokens(full_tokens, n_tokens, total_length, offset, duration):
    if len(full_tokens) < n_tokens:
        ...
        indices = [-1] * (n_tokens - len(full_tokens)) + \
            list(range(0, len(full_tokens)))
    else:
        assert 0 <= offset < total_length
        midpoint = int(len(full_tokens) * (offset +
                                           duration / 2.0) / total_length)
        midpoint = min(max(midpoint, n_tokens // 2),
                       len(full_tokens) - n_tokens // 2)
        tokens = full_tokens[midpoint - n_tokens // 2:midpoint + n_tokens // 2]
        indices = list(range(midpoint - n_tokens //
                             2, midpoint + n_tokens // 2))
    assert len(
        tokens) == n_tokens, f"Expected length {n_tokens}, got {len(tokens)}"
    assert len(
        indices) == n_tokens, f"Expected length {n_tokens}, got {len(indices)}"
    assert tokens == [full_tokens[index]
                      if index != -1 else 0 for index in indices]
...
class Labeller():
    ...
    def get_label(self, artist, genre, lyrics, total_length, offset):
        ...
        tokens, _ = get_relevant_lyric_tokens(
            full_tokens, self.n_tokens, total_length, offset, self.sample_length)

        assert len(genre_ids) <= self.max_genre_words
        genre_ids = genre_ids + [-1] * (self.max_genre_words - len(genre_ids))
        y = np.array([total_length, offset, self.sample_length,
                      artist_id, *genre_ids, *tokens], dtype=np.int64)
        assert y.shape == self.label_shape, f"Expected {self.label_shape}, got {y.shape}"
        info = dict(artist=artist, genre=genre,
                    lyrics=lyrics, full_tokens=full_tokens)
        return dict(y=y, info=info)
    ...
    def get_y_from_ids(self, artist_id, genre_ids, lyric_tokens, total_length, offset):
        ...
        if self.n_tokens > 0:
            ...
            lyric_tokens = []
        y = np.array([total_length, offset, self.sample_length,
                      artist_id, *genre_ids, *lyric_tokens], dtype=np.int64)
        assert y.shape == self.label_shape, f"Expected {self.label_shape}, got {y.shape}"
        return y
        ...
        if self.n_tokens > 0:
            ...
            for i in range(ys.shape[0]):
                ...
                tokens, indices = get_relevant_lyric_tokens(
                    full_tokens, self.n_tokens, total_length, offset, duration)
                tokens_list.append(tokens)
                indices_list.append(indices)
            ys[:, -self.n_tokens:] = t.tensor(tokens_list,
                                              dtype=t.long, device='cuda')
            return indices_list
        else:
            return None
    ...
    def describe_label(self, y):
        assert y.shape == self.label_shape, f"Expected {self.label_shape}, got {y.shape}"
        y = np.array(y).tolist()
        total_length, offset, length, artist_id, * \
            genre_ids = y[:4 + self.max_genre_words]
...
if __name__ == '__main__':
    ...
    label = labeller.get_label(
        "Alan Jackson", "Country Rock", "old town road", 4*60*44100, 0)
